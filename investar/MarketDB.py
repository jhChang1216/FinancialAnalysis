...
class MarketDB:
    ...
    def get_daily_price(self, code, start_date=None, end_date=None):
        ...
        if (end_date is None):
            ...
            if (end_lst[0] == ''):
                end_lst = int(end_lst[1:])
            end_year = int(end_lst[0])
            end_month = int(end_lst[1])
            end_day = int(end_lst[2])
            if end_year < 1800 or end_year> 2200:
                print(f"ValueError : end_year({end_year:d}) is unprocessable.")
                return
            if end_month < 1 or end_month > 12:
                print(f"ValueError : end_month({end_month:d}) is unprocessable.")
                return
            if end_day < 1 or end_day > 31:
                print(f"ValueError : end_day({end_day:d}) is unprocessable.")
                return
